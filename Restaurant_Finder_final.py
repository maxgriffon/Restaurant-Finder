...
if ID<0 or ID>=156:
    ...
    if option==1:
        webbrowser.open('{}'.format(restaurants[ID][4]))
    elif option==2:
        maps1=restaurants[ID][3].replace('+',', ')
        maps2=restaurants[ID][4].strip()
        maps3=maps1+maps2[0]
        webbrowser.open(('http://www.google.com/maps/place/{}').format(maps3))
    elif option==3:
        direc=restaurants[ID][3].replace('+',', ')
        direc1=restaurants[ID][4].strip()
        direc2= direc+direc1[0]
        webbrowser.open(('http://www.google.com/maps/place/110 8th Street, Troy NY/{}').format(direc2))